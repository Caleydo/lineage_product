...
function pushImages(images) {
  ...
  if (!argv.noDefaultTags) {
    ...
    if (publicDockerRegistry) {
      tags.push(...images.map((image) => ({image, tag: `index.docker.io/v1/caleydo/${cleanPublicName(image)}`})));
    }
  }
  if (argv.pushExtra) { //push additional custom prefix without the version
    ...
    if (publicDockerRegistry) {
      tags.push(...images.map((image) => ({
        image,
        tag: `index.docker.io/v1/caleydo/${cleanPublicName(image.substring(0, image.lastIndexOf(':')))}:${argv.pushExtra}`
      })));
    }
  }
  if (tags.length === 0) {
